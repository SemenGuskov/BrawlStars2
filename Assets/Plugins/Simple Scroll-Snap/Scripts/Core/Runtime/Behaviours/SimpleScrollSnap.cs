...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private MovementAxis movementAxis = MovementAxis.Horizontal_CV_256;
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            switch (movementAxis)
            {
                case MovementAxis.Horizontal_CV_256:
                    releaseDirection = (Velocity.x > 0) ? Direction.Right : Direction.Left;
                    break;
                case MovementAxis.Vertical_CV_256:
        ...
        private void Setup()
        {
            ...
            // Scroll-Rect
            ScrollRect.horizontal = (movementType == MovementType.Free) || (movementAxis == MovementAxis.Horizontal_CV_256);
            ScrollRect.vertical =   (movementType == MovementType.Free) || (movementAxis == MovementAxis.Vertical_CV_256);
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                {
                    Panels[i].anchorMin = new Vector2(movementAxis == MovementAxis.Horizontal_CV_256 ? 0f : 0.5f, movementAxis == MovementAxis.Vertical_CV_256 ? 0f : 0.5f);
                    Panels[i].anchorMax = new Vector2(movementAxis == MovementAxis.Horizontal_CV_256 ? 0f : 0.5f, movementAxis == MovementAxis.Vertical_CV_256 ? 0f : 0.5f);
                    ...
                    float panelPosX = (movementAxis == MovementAxis.Horizontal_CV_256) ? i * (automaticLayoutSpacing + 1f) * size.x + (size.x / 2f) : 0f;
                    float panelPosY = (movementAxis == MovementAxis.Vertical_CV_256)   ? i * (automaticLayoutSpacing + 1f) * size.y + (size.y / 2f) : 0f;
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                {
                    Content.anchorMin = new Vector2(movementAxis == MovementAxis.Horizontal_CV_256 ? 0f : 0.5f, movementAxis == MovementAxis.Vertical_CV_256 ? 0f : 0.5f);
                    Content.anchorMax = new Vector2(movementAxis == MovementAxis.Horizontal_CV_256 ? 0f : 0.5f, movementAxis == MovementAxis.Vertical_CV_256 ? 0f : 0.5f);
                    Content.pivot = new Vector2(movementAxis == MovementAxis.Horizontal_CV_256     ? 0f : 0.5f, movementAxis == MovementAxis.Vertical_CV_256 ? 0f : 0.5f);

                    Vector2 min = Panels[0].anchoredPosition;
                    Vector2 max = Panels[NumberOfPanels - 1].anchoredPosition;

                    float contentWidth  = (movementAxis == MovementAxis.Horizontal_CV_256) ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * size.x) - (size.x * automaticLayoutSpacing) : size.x;
                    float contentHeight = (movementAxis == MovementAxis.Vertical_CV_256)   ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * size.y) - (size.y * automaticLayoutSpacing) : size.y;
            ...
            // Starting Panel
            float xOffset = (movementType == MovementType.Free || movementAxis == MovementAxis.Horizontal_CV_256) ? Viewport.rect.width  / 2f : 0f;
            float yOffset = (movementType == MovementType.Free || movementAxis == MovementAxis.Vertical_CV_256)   ? Viewport.rect.height / 2f : 0f;
        ...
        private void HandleTransitionEffects()
        {
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                float d = (movementType == MovementType.Free) ? displacement.magnitude : ((movementAxis == MovementAxis.Horizontal_CV_256) ? displacement.x : displacement.y);
